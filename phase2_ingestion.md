...
Create the file `ingest.py` with the code below.  
It downloads a YouTube video, extracts frames, describes them with Gemini, embeds them, and stores everything in ChromaDB.
...
```bash
python -c "from ingest import ingest; c, col = ingest('https://youtu.be/dQw4w9WgXcQ'); print(col.count())"
```
Should print >0.

### Git commit & push
```bash
git add ingest.py index.json chroma_db/ videos/ frames/ .gitignore 2>/dev/null
git commit -m "Phase 2: ingestion pipeline (download, extract, describe, embed, store)"
git push origin main
```

Next: phase3_query.md.